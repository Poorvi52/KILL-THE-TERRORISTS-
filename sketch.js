...
function draw() {
  ...
  if(gameState === play) {
    ...
    text("Terrorists have attacked India! Good luck next time!",canvas.width/2-400,canvas.height/2+100);
  ...
  if(score == 25){
    ...
    fill("black");
    textSize(40);
    text("You protected your planet from the terrorists! Great Job.",canvas.width/2-350,canvas.height/2);
  ...
  if(gameState === instruction) {
    ...
    text(" Shoot 25 terrorists to protect India and its residents from terrorists.",canvas.width/2-300,canvas.height-250);
...
function  terrorists() {
  if(frameCount % 60 === 0) {
    ...
    terrorist.scale = random(0.3,0.5);
